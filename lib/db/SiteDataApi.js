...
async function convertNotionToSiteData(
  ...
  if (sortBy === 'date') {
    allPages.sort((a, b) => {
      const dateA = Number(b?.lastEditedDate || b?.publishDate || 0) || 0
      const dateB = Number(a?.lastEditedDate || a?.publishDate || 0) || 0
      return dateA - dateB
    })
    // 诊断日志：打印最新3篇文章的日期信息
    if (process.env.NODE_ENV === 'development') {
      const posts = allPages.filter(p => p?.type === 'Post' && p?.status === 'Published')
      console.log('[SORT DIAGNOSTIC] Top 3 posts by date:')
      posts.slice(0, 3).forEach(p => {
        console.log('  -', p?.title, '| publishDate:', p?.publishDate, '| lastEditedDate:', p?.lastEditedDate, '| dateField:', p?.date)
      })
    }
  }
